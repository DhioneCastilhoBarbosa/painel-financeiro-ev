...
### ⚡ Eletropostos — `https://app.powerbi.com/view?r=...` do BI Eletropostos
```js
(() => {
  const RKEY = '1c21f0be-a53a-4aed-8aa8-2bafd3ac52c8';
  const OUT  = 'eletropostos_por_municipio.csv';
  __abveCapture(RKEY, OUT);
})();
```

### 🚗 Frotas — `https://app.powerbi.com/view?r=...` do BI Frotas
```js
(() => {
  const RKEY = '2876fadc-cffd-4d01-bf73-446d1bbbcb45';
  const OUT  = 'frota_ev_por_municipio.csv';
  __abveCapture(RKEY, OUT);
})();
```

### 🔧 Cole ESTE primeiro (define `__abveCapture`, serve para os dois painéis)
```js
