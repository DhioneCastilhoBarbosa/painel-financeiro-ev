...
def _plan(
    id: str, name: str, price_brl: int, price_label: str,
    max_users: int, max_files: int, is_public: bool,
    features: list[str], flags: dict[str, bool],
) -> dict[str, Any]:
    return {
        "id": id, "name": name, "price_brl": price_brl,
        "price_label": price_label, "max_users": max_users,
        "max_files": max_files, "is_public": is_public,
        "stripe_price_id": None, "features": features, "feature_flags": flags,
    }
...
_DEFAULTS: dict[str, Any] = {
    "plans": [
        _plan(
            id="trial", name="Trial", price_brl=0, price_label="Gratuito",
            max_users=3, max_files=5, is_public=False,
            features=[
                "3 usuários", "5 arquivos de dados",
                "Dashboard, Receita, Estações, DRE",
                "Análise de Investimento Simplificada",
                "CAPEX por Carregador",
                "Exportação CSV", "Suporte por e-mail",
            ],
            flags={
                **_ALL_FLAGS_OFF,
        ...
        _plan(
            id="starter", name="Starter", price_brl=19700, price_label="R$ 197/mês",
            max_users=3, max_files=5, is_public=True,
            features=[
                "3 usuários", "5 arquivos de dados",
        ...
        _plan(
            id="pro", name="Pro", price_brl=49700, price_label="R$ 497/mês",
            max_users=10, max_files=30, is_public=True,
            features=[
                "10 usuários", "30 arquivos de dados",
                "Todos os dashboards + Mapa de Instalação EV",
                "Análise Simplificada e Avançada (VPL, TIR, sensibilidade)",
                "Relatório PDF", "Leads (CRM)",
                "Exportação CSV + PDF", "Suporte prioritário",
            ],
            flags={
                **_ALL_FLAGS_OFF,
                "dashboard_overview":  True,
        ...
        _plan(
            id="enterprise", name="Enterprise", price_brl=0, price_label="Sob consulta",
            max_users=9999, max_files=9999, is_public=False,
            features=[
                "Usuários ilimitados", "Arquivos ilimitados",
                "Todos os módulos", "Análise Avançada completa",
                "Mapa de Instalação EV", "Leads (CRM)",
                "Acesso à API", "Suporte dedicado", "SLA garantido",
            ],
            flags={k: True for k in _ALL_FLAGS_OFF},
        ),
        _plan(
            id="free", name="Free", price_brl=0, price_label="Gratuito",
            max_users=1, max_files=1, is_public=False,
            features=["1 usuário", "1 arquivo", "Dashboard básico"],
            flags={
                **_ALL_FLAGS_OFF,
                "dashboard_overview": True,
                "billing":            True,
